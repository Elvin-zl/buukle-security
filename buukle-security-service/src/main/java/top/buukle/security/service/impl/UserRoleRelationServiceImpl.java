...
import top.buukle.security .dao.UserRoleRelationMapper;
...
import top.buukle.security .service.constants.UserRoleRelationEnums;
...
/**
* @author elvin
* @description UserRoleRelationService实现类
*/
...
public class UserRoleRelationServiceImpl implements UserRoleRelationService{
    ...
    public PageResponse getPage(BaseQuery query) {
        PageHelper.startPage(((UserRoleRelationQuery)query).getPage(),((UserRoleRelationQuery)query).getPageSize());
        List<UserRoleRelation> list = userRoleRelationMapper.selectByExample(this.assExampleForList(((UserRoleRelationQuery)query)));
        PageInfo<UserRoleRelation> pageInfo = new PageInfo<>(list);
    ...
    public CommonResponse delete(Integer id, HttpServletRequest request, HttpServletResponse response) {
        if(userRoleRelationMapper.updateByPrimaryKeySelective(this.assQueryForUpdateStatus(id, UserRoleRelationEnums.status.DELETED.value(),request,response)) != 1){
    ...
    public CommonResponse deleteBatch(String ids, HttpServletRequest request, HttpServletResponse response) {
        ...
        userRoleRelation.setModifier(operator.getUsername());
        userRoleRelation.setModifierCode(operator.getUserId());

        userRoleRelation.setStatus(UserRoleRelationEnums.status.DELETED.value());
        userRoleRelationMapper.updateByExampleSelective(userRoleRelation,userRoleRelationExample);
        return new CommonResponse.Builder().buildSuccess();
    }
     ...
     * @description 根据id查询
     * @param id
     * @return top.buukle.security.entity.UserRoleRelation
     * @Author elvin
     * @Date 2019/8/4
     */
    @Override
    public UserRoleRelation selectByPrimaryKeyForCrud(HttpServletRequest httpServletRequest, Integer id) {
        ...
        return userRoleRelation == null ? new UserRoleRelation() : userRoleRelation;
    }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED,isolation= Isolation.DEFAULT ,rollbackFor = Exception.class)
    public CommonResponse saveOrEdit(UserRoleRelationQuery query, HttpServletRequest request, HttpServletResponse response) {
    ...
    public FuzzyResponse fuzzySearch(String text, String fieldName) {
        ...
        fuzzyVo.setTableName(ConvertHumpUtil.humpToLine("UserRoleRelation"));
    ...
    public CommonResponse save(BaseQuery query, HttpServletRequest request, HttpServletResponse response) {
        ...
        userRoleRelationMapper.insert(this.assQueryForInsert((UserRoleRelationQuery)query,request,response));
        return new CommonResponse.Builder().buildSuccess();
    }
    ...
    public CommonResponse update(BaseQuery query, HttpServletRequest request, HttpServletResponse response) {
        UserRoleRelationQuery userRoleRelationQuery = ((UserRoleRelationQuery)query);

        UserRoleRelationExample example = new UserRoleRelationExample();
        UserRoleRelationExample.Criteria criteria = example.createCriteria();
        criteria.andIdEqualTo(userRoleRelationQuery.getId());
        User operator = SessionUtil. getOperator(request, response);
        userRoleRelationQuery.setGmtModified(new Date());
        userRoleRelationQuery.setModifier(operator.getUsername());
        userRoleRelationQuery.setModifierCode(operator.getUserId());
        userRoleRelationMapper.updateByExampleSelective(userRoleRelationQuery,example);
        return new CommonResponse.Builder().buildSuccess();
    }
     ...
     * @Date 2019/8/5
     */
    private void validateParamForSaveOrEdit(UserRoleRelationQuery query) {
        // TODO
    }
     ...
     * @return
     */
    private UserRoleRelation assQueryForInsert(UserRoleRelationQuery query, HttpServletRequest request, HttpServletResponse response) {
        this.validateParamForSaveOrEdit(query);
        query.setStatus(UserRoleRelationEnums.status.INIT.value());
     ...
     * @return
     */
    private UserRoleRelation assQueryForUpdateStatus(Integer id, Integer status, HttpServletRequest request, HttpServletResponse response) {
        UserRoleRelationQuery infoQuery = new UserRoleRelationQuery();
        User operator = SessionUtil. getOperator(request, response);
        infoQuery.setId(id);
     ...
     * @return
     */
    private UserRoleRelationExample assExampleForList(UserRoleRelationQuery query) {
        UserRoleRelationExample example = new UserRoleRelationExample();
        UserRoleRelationExample.Criteria criteria = example.createCriteria();
